...
inline
Window_mgr::ScreenIndex //因为这里是类作用域之外，所以说明返回类型时必须带上类的名字，指定返回的是Window_mgr中的ScreenIndex。
Window_mgr::addScreen(const Screen &s) //上面一行是指明返回类型，这一行才是进入Window_mgr的作用域内，定义一个addScreen函数。
{
	screens.push_back(s);
